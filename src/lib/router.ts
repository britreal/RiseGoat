...
export type Route =
  ...
  | { name: 'launches' } | { name: 'radar' } | { name: 'goals' } | { name: 'goat' } | { name: 'flux'; workflowId?: string } | { name: 'admin' }
...
function parseRoute(): Route {
  ...
  const directBlog = getBlogRoute(window.location.pathname);
  ...
  if (parts[0] === 'public-blog' && parts[1]) return { name: 'public-blog', slug: decodeURIComponent(parts[1]) };
  if (parts[0] === 'public-blog-post' && parts[1] && parts[2]) return { name: 'public-blog-post', slug: decodeURIComponent(parts[1]), postSlug: decodeURIComponent(parts[2]) };
  ...
  if (parts[0] === 'flux') return { name: 'flux', workflowId: parts[1] };
...
export function useRouter() {
  ...
  const navigate = useCallback((path: string) => {
    if (path.startsWith('/@') || path.startsWith('/u/') || path.startsWith('/p/') || path.startsWith('/unsubscribe/') ||
      (getBlogRoute(path) && !path.startsWith('/#'))) {
