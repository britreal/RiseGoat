...
function download(data:Uint8Array|Blob,name:string,type:string){const blob=data instanceof Blob?data:new Blob([data],{type});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=name;a.click();window.setTimeout(()=>URL.revokeObjectURL(url),1000);return blob;}
...
export function buildEpub(book:{title:string;subtitle:string;description:string;author_name:string;language:string;isbn:string},chapters:{title:string;content:string}[]){
  ...
  const chaptersXml=items.map(x=>'<?xml version="1.0" encoding="utf-8"?><html xmlns="http://www.w3.org/1999/xhtml"><head><title>'+xml(x.title)+'</title><link rel="stylesheet" href="style.css"/></head><body><h1>'+xml(x.title)+'</h1><p>'+x.content+'</p></body></html>');
  ...
  return download(data,safeName(book.title)+'.epub','application/epub+zip');
}
...
export function buildPdf(book:{title:string;author_name:string},chapters:{title:string;content:string}[]){
  ...
  line(book.title);if(book.author_name)line(book.author_name);y-=15;chapters.forEach(ch=>{y-=10;line(ch.title);line(ch.content);y-=8;});if(page.length)pages.push(page);
  const objects:string[]=['','', '<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>'];const pageIds:number[]=[];
  pages.forEach(p=>{const pageId=objects.length+1,contentId=objects.length+2;pageIds.push(pageId);objects.push('<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] /Resources << /Font << /F1 3 0 R >> >> /Contents '+contentId+' 0 R >>');const stream='BT\\n/F1 11 Tf\\n'+p.map(x=>{const [yy,...rest]=x.split('|');return '1 0 0 1 48 '+yy+' Tm ('+pdfEscape(rest.join('|'))+') Tj';}).join('\\n')+'\\nET';objects.push('<< /Length '+stream.length+' >>\\nstream\\n'+stream+'\\nendstream');});
  objects[0]='<< /Type /Catalog /Pages 2 0 R >>';objects[1]='<< /Type /Pages /Kids ['+pageIds.map(x=>x+' 0 R').join(' ')+'] /Count '+pageIds.length+' >>';
  let pdf='%PDF-1.4\\n';const offsets:number[]=[0];objects.forEach((o,i)=>{offsets.push(pdf.length);pdf+=(i+1)+' 0 obj\\n'+o+'\\nendobj\\n';});const xref=pdf.length;pdf+='xref\\n0 '+(objects.length+1)+'\\n0000000000 65535 f \\n';for(let i=1;i<offsets.length;i++)pdf+=String(offsets[i]).padStart(10,'0')+' 00000 n \\n';pdf+='trailer\\n<< /Size '+(objects.length+1)+' /Root 1 0 R >>\\nstartxref\\n'+xref+'\\n%%EOF';
