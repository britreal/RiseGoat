...
function Cell(props:{children:React.ReactNode;strong?:boolean}) { return <td className={'px-4 py-3 text-sm ' + (props.strong?'font-medium text-slate-800':'text-slate-500')}>{props.children}</td>; }function StrategyPanel(props:{
  ...
  const scored=props.nodes.map((node)=>{
    ...
    const score=(connectionScore*0.3)+(connectionScore*0.3)+(decision*0.2)+(resources*0.2);
    return {...node,score};
  ...
  async function saveHidden(){if(!userId||!personA.trim()||!personB.trim())return;const {error}=await supabase.from('hidden_connections').insert({user_id:userId,person_a:personA,person_b:personB,relation,source,strength});if(error)return;setPersonA('');setPersonB('');setRelation('');setSource('');props.onRefresh();}
  async function saveInfluence(){if(!userId||!influencer.trim()||!influenced.trim())return;const {error}=await supabase.from('cross_influence').insert({user_id:userId,influencer,influenced,intensity:Number(intensity||5),topic});if(error)return;setInfluencer('');setInfluenced('');setTopic('');props.onRefresh();}
  const currentNegotiation=props.negotiations.find((x)=>x.contact_id===selected);
  useEffect(()=>{if(!currentNegotiation)return;setHistory(currentNegotiation.history||'');setStyle(currentNegotiation.style||'');setLimits(currentNegotiation.limits||'');setTriggers(currentNegotiation.triggers||'');setAlternatives(currentNegotiation.alternatives||'');},[currentNegotiation?.id]);
