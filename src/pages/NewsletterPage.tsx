...
export function NewsletterPage() {
  ...
  async function sendBroadcast() {
    const subscribedLeads = leads.filter((lead) => lead.marketing_consent && !lead.unsubscribed_at);
    if (!user || !subject.trim() || !body.trim() || !subscribedLeads.length) return;
    setSending(true); setSent(false); setSendError(null);
    const { data: connectionStatus, error: connectionError } = await supabase.functions.invoke('get-email-connection', { body: { user_id: user.id } });
  ...
  return (
    <div className="p-6 lg:p-8 max-w-3xl mx-auto">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
        <Card className="p-4"><div className="flex items-center gap-2 mb-1"><Users className="w-4 h-4 text-cyan-500"/><span className="text-xs text-slate-500">Inscritos</span></div><p className="text-2xl font-bold text-slate-900">{leads.filter(l=>l.marketing_consent && !l.unsubscribed_at).length}</p></Card>
      ...
      <Card className="p-5 mb-6">
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button onClick={sendBroadcast} disabled={!subject.trim()||!body.trim()||sending||!leads.length} className="flex items-center gap-2 px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white text-sm font-medium rounded-lg disabled:opacity-50">{sending?<Loader2 className="w-4 h-4 animate-spin"/>:<Mail className="w-4 h-4"/>}{sending?'Enviando...':`Enviar para ${leads.filter(l=>l.marketing_consent && !l.unsubscribed_at).length} inscritos`}</button>
          ...
          {leads.filter(l=>l.marketing_consent && !l.unsubscribed_at).length===0 && <p className="text-xs text-slate-400">Nenhum lead com consentimento de marketing ativo para envio.</p>}
        </div>
      </Card>
