...
export function DashboardLayout({ children, currentPath, navigate }: DashboardLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex">
      ...
      <aside
        className={cn(
          'fixed lg:sticky top-0 left-0 h-screen w-64 bg-white border-r border-slate-200 flex flex-col z-50 transition-transform',
      ...
      >
        <div className="flex items-center gap-2 px-5 h-16 border-b border-slate-200">
          ...
          <span className="font-bold text-slate-800 tracking-tight">Simply Connect</span>
        </div>

        <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-5">
          {groups.map((group) => (
            <div key={group}>
              <p className="px-3 mb-1.5 text-[10px] font-semibold uppercase tracking-wider text-slate-400">{group}</p>
        ...
        <div className="border-t border-slate-200 p-3 space-y-1">
          {profile && (
            <button
              ...
              className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-slate-600 hover:bg-slate-100 transition"
            >
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <header className="lg:hidden sticky top-0 z-30 bg-white border-b border-slate-200 h-14 flex items-center px-4 gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-slate-800 text-sm">Simply Connect</span>
          </div>
        </header>
